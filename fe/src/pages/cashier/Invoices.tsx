...
const CashierInvoices: FC = () => {
  ...
  const [formData, setFormData] = useState<{
    ...
    TongTien: "",
    TrangThai: 'Done', // Mặc định là chờ thanh toán
  ...
  const handleCreateInvoiceFromMedicalForm = async () => {
    ...
    try {
      ...
      const invoiceData = {
        ...
        TrangThai: 'Done' as const, // Tạo hóa đơn chờ thanh toán
      ...
      // Cập nhật trạng thái phiếu thành "Payed"
      try {
        const updateData = {
          NgayYeuCau: searchedMedicalForm.NgayYeuCau,
          DonGia: searchedMedicalForm.DonGia.toString(),
          Loai: searchedMedicalForm.Loai,
          MaBN: searchedMedicalForm.MaBN,
          MaDichVu: searchedMedicalForm.MaDichVu,
          TrangThai: 'Payed' as const
        };
        
        await medicalFormAPI.updatePhieu(searchedMedicalForm.MaPYC, updateData);
        console.log("Updated medical form status to Payed");
      } catch (updateError) {
        console.error("Error updating medical form status:", updateError);
        // Vẫn hiển thị thông báo thành công vì hóa đơn đã được tạo
        showNotification("Hóa đơn đã tạo thành công, nhưng có lỗi khi cập nhật trạng thái phiếu", "warning");
        return; // Dừng lại nếu không cập nhật được trạng thái
      }
  ...
  const handleCreateInvoiceFromPrescription = async () => {
    ...
    try {
      ...
      // Cập nhật trạng thái toa thuốc thành "Payed"
      try {
        const updateData = {
          MaBN: searchedPrescription.MaBN,
          NgayKe: searchedPrescription.NgayKe,
          TrangThai: 'Payed' as const
        };
        
        await toathuocAPI.updateToaThuoc(searchedPrescription.MaToaThuoc, updateData);
        console.log("Updated prescription status to Payed");
      } catch (updateError) {
        console.error("Error updating prescription status:", updateError);
        showNotification("Hóa đơn đã tạo thành công, nhưng có lỗi khi cập nhật trạng thái toa thuốc", "warning");
        return; // Dừng lại nếu không cập nhật được trạng thái
      }
