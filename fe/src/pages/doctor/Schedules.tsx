...
interface ScheduleFormData {
  ...
  Buoi: 'Sang' | 'Chieu'; // Backend chỉ hỗ trợ 2 buổi
  SoBNToiDa: number;
  Gia: string; // Backend yêu cầu string
  MaNV: string;
}
...
const DoctorSchedules: FC = () => {
  ...
  const handleCreateSchedule = async () => {
    try {
      ...
      const scheduleData = {
        SoBNHienTai: formData.SoBNHienTai, // Backend yêu cầu field này
        Ngay: formData.Ngay,
        Buoi: formData.Buoi,
  ...
  const handleEditSchedule = (schedule: Schedule) => {
    ...
    setFormData({
      ...
      Gia: schedule.Gia, // Giữ nguyên string
      MaNV: schedule.MaNV
    });
                  ...
                  <TableBody>
                    {schedules.map((schedule) => (
                      <TableRow key={schedule.MaLich} hover>
                        ...
                        <TableCell>
                          <Typography variant="body2" sx={{ color: "#094067", fontWeight: 600 }}>
                            {parseInt(schedule.Gia).toLocaleString('vi-VN')} ₫
                          </Typography>
                        </TableCell>
                        <TableCell>
                          <Typography variant="body2" color="#5f6c7b">
